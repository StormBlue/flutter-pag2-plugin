...
static NSString *const methodSetVideoEnabled = @"setVideoEnabled";
static NSString *const methodSetUseDiskCache = @"setUseDiskCache";
static NSString *const methodSetCacheScale = @"setCacheScale";
static NSString *const methodSetMaxFrameRate = @"setMaxFrameRate";
...
static NSString *const argumentFrameAvailable = @"frameAvailable";
static NSString *const argumentVideoEnabled = @"videoEnabled";
static NSString *const argumentUseDiskCache = @"useDiskCache";
static NSString *const argumentCacheScale = @"cacheScale";
static NSString *const argumentMaxFrameRate = @"maxFrameRate";
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel methodChannelWithName:@"flutter_pag2_plugin"
...
- (void)release:(id)arguments result:(FlutterResult _Nonnull)result {
    ...
    BOOL frameAvailable = YES;
    if (arguments[argumentFrameAvailable]) {
        frameAvailable = [[arguments objectForKey:argumentFrameAvailable] boolValue];
    }

    // 防止并行加入freeEntryPool超过maxFreePoolSize，且仅缓存已正常渲染的render
    BOOL shouldAddToFreePool = self.enableRenderCache && self.preFreeEntryPool.count < self.maxFreePoolSize && frameAvailable;
...
- (void)setVideoEnabled:(id)arguments result:(FlutterResult _Nonnull)result {
    NSNumber *textureId = arguments[argumentTextureId];
    if(!textureId || [textureId compare:@0] == NSOrderedAscending){
        result(@-1);
        return;
    ...
    id value = arguments[argumentVideoEnabled];
    if ([value isKindOfClass:[NSNumber class]]) {
        videoEnabled = [value boolValue];
    }
    TGFlutterPagRender *render = [_renderMap objectForKey:textureId];
...
- (void)setUseDiskCache:(id)arguments result:(FlutterResult _Nonnull)result {
    NSNumber *textureId = arguments[argumentTextureId];
    if(!textureId || [textureId compare:@0] == NSOrderedAscending){
        result(@-1);
        return;
    }
    BOOL useDiskCache = NO;
    id value = arguments[argumentUseDiskCache];
    if ([value isKindOfClass:[NSNumber class]]) {
        useDiskCache = [value boolValue];
    }
    TGFlutterPagRender *render = [_renderMap objectForKey:textureId];
...
- (void)setCacheScale:(id)arguments result:(FlutterResult _Nonnull)result {
    NSNumber *textureId = arguments[argumentTextureId];
    if(!textureId || [textureId compare:@0] == NSOrderedAscending){
        result(@-1);
        return;
    ...
    id value = arguments[argumentCacheScale];
    if ([value isKindOfClass:[NSNumber class]]) {
        cacheScale = [value floatValue];
    }
    TGFlutterPagRender *render = [_renderMap objectForKey:textureId];
...
- (void)setMaxFrameRate:(id)arguments result:(FlutterResult _Nonnull)result {
    NSNumber *textureId = arguments[argumentTextureId];
    if(!textureId || [textureId compare:@0] == NSOrderedAscending){
        result(@-1);
        return;
    ...
    id value = arguments[argumentMaxFrameRate];
    if ([value isKindOfClass:[NSNumber class]]) {
        maxFrameRate = [value floatValue];
    }
    TGFlutterPagRender *render = [_renderMap objectForKey:textureId];
...
- (void)initPag:(id)arguments result:(FlutterResult _Nonnull)result {
    ...
    NSNumber *videoEnabled = [arguments objectForKey:argumentVideoEnabled];
    if (![videoEnabled isKindOfClass:[NSNumber class]]) {
        videoEnabled = nil;
    }
    NSNumber *useDiskCache = [arguments objectForKey:argumentUseDiskCache];
    if (![useDiskCache isKindOfClass:[NSNumber class]]) {
        useDiskCache = nil;
    }
    NSNumber *cacheScale = [arguments objectForKey:argumentCacheScale];
    if (![cacheScale isKindOfClass:[NSNumber class]]) {
        cacheScale = nil;
    }
    NSNumber *maxFrameRate = [arguments objectForKey:argumentMaxFrameRate];
    if (![maxFrameRate isKindOfClass:[NSNumber class]]) {
        maxFrameRate = nil;
    }
    ...
    if (_enableReuse && reuse){
        ...
        if (reuseKey && [reuseKey length] > 0){
            ReuseItem *reuseItem = [_reuseMap objectForKey:reuseKey];
            BOOL existReuseRender = NO;
            if (reuseItem) {
                NSNumber *reuseTextureId = [reuseItem getTextureId];
                NSComparisonResult comparisonResult = [reuseTextureId compare:@0];
                existReuseRender = (comparisonResult == NSOrderedSame || comparisonResult == NSOrderedDescending) && [_renderMap objectForKey:reuseTextureId];
            }
...
- (void)enableReuse:(id)arguments result:(FlutterResult _Nonnull)result {
    BOOL enable = NO; // 默认关闭 enableReuse render 复用
    if ([arguments isKindOfClass:[NSDictionary class]]) {
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    } else if([methodSetVideoEnabled isEqualToString:call.method]){
        [self setVideoEnabled:arguments result:result];
    } else if([methodSetUseDiskCache isEqualToString:call.method]){
        [self setUseDiskCache:arguments result:result];
    } else if([methodSetCacheScale isEqualToString:call.method]){
        [self setCacheScale:arguments result:result];
    } else if([methodSetMaxFrameRate isEqualToString:call.method]){
        [self setMaxFrameRate:arguments result:result];
    } else {
...
-(void)pagRenderWithPagData:(NSData *)pagData
               ...
               videoEnabled:(NSNumber *)videoEnabled
               useDiskCache:(NSNumber *)useDiskCache
                 cacheScale:(NSNumber *)cacheScale
               maxFrameRate:(NSNumber *)maxFrameRate
                     result:(FlutterResult)result
                      reuse:(bool)reuse
                   reuseKey:(NSString *)reuseKey
                     viewId:(int)viewId{
    ...
    [[TGFlutterWorkerExecutor sharedInstance] post:^(){
        ...
        dispatch_async(dispatch_get_main_queue(), ^(){
            if (videoEnabled) {
                [render setVideoEnabled:[videoEnabled boolValue]];
            }
            if (useDiskCache) {
                [render setUseDiskCache:[useDiskCache boolValue]];
            }
            if (cacheScale) {
                [render setCacheScale:[cacheScale floatValue]];
            }
            if (maxFrameRate) {
                [render setMaxFrameRate:[maxFrameRate floatValue]];
            }
            if (autoPlay) {
                [render startRender];
            }
