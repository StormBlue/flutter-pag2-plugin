...
public class FlutterPagPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    final static String _nativeSetVideoEnabled = "setVideoEnabled";
    final static String _nativeSetUseDiskCache = "setUseDiskCache";
    final static String _nativeSetCacheScale = "setCacheScale";
    final static String _nativeSetMaxFrameRate = "setMaxFrameRate";
    ...
    final static String _argumentVideoEnabled = "videoEnabled";
    final static String _argumentUseDiskCache = "useDiskCache";
    final static String _argumentCacheScaleValue = "cacheScale";
    final static String _argumentMaxFrameRateValue = "maxFrameRate";
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding binding) {
        ...
        channel = new MethodChannel(binding.getBinaryMessenger(), "flutter_pag2_plugin");
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        switch (call.method) {
            ...
            case _nativeEnableReuse:
                ...
                result.success("");
                break;
            case _nativeSetVideoEnabled:
                setVideoEnabled(call);
                result.success("");
                break;
            case _nativeSetUseDiskCache:
                setUseDiskCache(call);
                result.success("");
                break;
            case _nativeSetCacheScale:
                setCacheScale(call);
                result.success("");
                break;
            case _nativeSetMaxFrameRate:
                setMaxFrameRate(call);
                result.success("");
                break;
            default:
    ...
    private void setVideoEnabled(final MethodCall call) {
        final Boolean videoEnabled = call.argument(_argumentVideoEnabled);
        FlutterPagPlayer flutterPagPlayer = getFlutterPagPlayer(call);
        if (flutterPagPlayer != null && videoEnabled != null) {
            flutterPagPlayer.setVideoEnabled(videoEnabled);
        }
    }

    private void setUseDiskCache(final MethodCall call) {
        final Boolean useDiskCache = call.argument(_argumentUseDiskCache);
        FlutterPagPlayer flutterPagPlayer = getFlutterPagPlayer(call);
        if (flutterPagPlayer != null && useDiskCache != null) {
            flutterPagPlayer.setUseDiskCache(useDiskCache);
        }
    }

    private void setCacheScale(final MethodCall call) {
        final Number cacheScale = call.argument(_argumentCacheScaleValue);
        FlutterPagPlayer flutterPagPlayer = getFlutterPagPlayer(call);
        if (flutterPagPlayer != null && cacheScale != null) {
            flutterPagPlayer.setCacheScale(cacheScale.floatValue());
        }
    }

    private void setMaxFrameRate(final MethodCall call) {
        final Number maxFrameRate = call.argument(_argumentMaxFrameRateValue);
        FlutterPagPlayer flutterPagPlayer = getFlutterPagPlayer(call);
        if (flutterPagPlayer != null && maxFrameRate != null) {
            flutterPagPlayer.setMaxFrameRate(maxFrameRate.floatValue());
        }
    }
    ...
    private void initPag(final MethodCall call, final Result result) {
        ...
        if (bytes != null) {
            ...
            DataLoadHelper.loadPag(url, bytes2 -> handler.post(() -> {
                if (bytes2 == null) {
                    error(call, result, "-1100", "url资源加载错误: " + url, null);
                    return;
                }

                initPagPlayerAndCallback(PAGFile.Load(bytes2), call, result);
    ...
    private void initPagPlayerAndCallback(PAGFile composition, MethodCall call, final Result result) {
        ...
        if (freeEntryPool.isEmpty() || !useCache) {
            ...
            entry.setOnFrameConsumedListener(() -> {
                if (!isFirstCall[0]) return;
                isFirstCall[0] = false;
                handler.post(() -> notifyFrameReady(entry.id(), viewId));
                entry.setOnFrameConsumedListener(null);
            });
        ...
        WorkThreadExecutor.getInstance().post(() -> {
            ...
            if (videoEnabled != null) {
                pagPlayer.setVideoEnabled(videoEnabled);
            }
            if (useDiskCache != null) {
                pagPlayer.setUseDiskCache(useDiskCache);
            }
            if (cacheScale != null) {
                pagPlayer.setCacheScale(cacheScale.floatValue());
            }
            if (maxFrameRate != null) {
                pagPlayer.setMaxFrameRate(maxFrameRate.floatValue());
            }
